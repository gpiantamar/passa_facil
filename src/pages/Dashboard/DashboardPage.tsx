...
export function DashboardPage() {
  ...
  const chartData = period === "7days" ? data.revenueByDay : data.revenueByMonth;
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full min-w-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <StatCard
          title="Pendentes"
          value={String(data.stats.pendingServices)}
          icon={Clock}
          iconBg="bg-amber-50"
        ...
        <StatCard
          title="Em andamento"
          value={String(data.stats.inProgressServices)}
          icon={Shirt}
          iconBg="bg-blue-50"
        ...
        <StatCard
          title="Prontos"
          value={String(data.stats.readyServices)}
          icon={CheckCircle2}
        ...
        <StatCard
          title="A receber"
          value={formatCurrency(data.stats.totalReceivable)}
          icon={DollarSign}
          iconBg="bg-orange-50"
        ...
        <StatCard
          ...
          value={String(data.stats.totalPiecesProcessed)}
          icon={Package}
          iconBg="bg-purple-50"
          ...
          <div className="flex flex-col gap-2">
            {data.alerts.map((alert) => (
              <button
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left w-full hover:opacity-85 transition-opacity ${alertBg[alert.type]}`}
              >
      ...
      <Card>
        ...
        <div className="h-48 w-full min-w-0">
          {hasRevenueData || chartData.length > 0 ? (
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="text-xs text-indigo-600 font-medium hover:text-indigo-700 flex items-center gap-0.5 cursor-pointer"
          >
            Ver todos <ArrowRight className="w-3 h-3 flex-shrink-0" />
          </button>
        </div>
        {data.todayServices.length === 0 ? (
          <div className="py-8 text-center text-slate-400">
            <p className="text-sm">Nenhum serviço agendado para hoje.</p>
          </div>
        ) : (
          <div className="flex flex-col divide-y divide-slate-50">
            {data.todayServices.map((service) => (
              <button
                key={service.id}
                onClick={() => navigate(`/servicos/${service.id}`)}
                className="flex items-center gap-3 py-3 text-left hover:bg-slate-50 -mx-1 px-1 rounded-xl transition-colors cursor-pointer"
              >
      ...
      <Card>
        ...
        {data.recentServices.length === 0 ? (
          <div className="py-8 text-center text-slate-400">
            <p className="text-sm">Nenhum serviço registrado recentemente.</p>
          </div>
        ) : (
          <>
            {/* Desktop table */}
            ...
            <div className="sm:hidden flex flex-col divide-y divide-slate-50">
              {data.recentServices.map((service) => (
                <button
                  key={service.id}
                  onClick={() => navigate(`/servicos/${service.id}`)}
                  className="flex items-center gap-3 py-3 text-left w-full cursor-pointer"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-mono text-slate-400">
